...
///////////////////////////////////////////////////////////
    // 添加一个函数来获取多项式的头结点
    Node<PolyItem> *getHead() const
    {
        return polyList.getHead();
    }
///////////////////////////////////////////////////////////
...
{
	LinkList<PolyItem> la = this->polyList;			// 多项式p对应的线性表
	LinkList<PolyItem> lc;						// 和多项式对应的线性表
	int aPos = 1,cPos=1;
	PolyItem aItem,cItem,expn_same_Item;//万一输入多项式时输入了多个次数相同的项，则需要将次数相同项求导后的系数合并，此处expn_same_Item的作用是记录相同次方的项
	...
	while (aStatus == ENTRY_FOUND)
	{
		...
		expn_same_Item.coef = expn_same_Item.coef * expn_same_Item.expn;//求导项系数处理
		expn_same_Item.expn -= 1;//指数处理
		if (expn_same_Item.coef == 0)//如果系数等于零则直接进入下一次循环
		{
			aStatus = la.GetElem(aPos++, aItem);
			continue;
		}
		aStatus = la.GetElem(aPos++, aItem);
		if (expn_same_Item.expn + 1 == aItem.expn&& aStatus == ENTRY_FOUND)//既要判断次数是否相同也要注意可能已经遍历到了末节点
		{
...
// 操作结果：返回当前多项式与p之差——减法运算符重载
{
	...
	while (aStatus == ENTRY_FOUND && bStatus == ENTRY_FOUND) {
		if (aItem.expn > bItem.expn) {		// la中的项aItem指数较小
			...
			lc.InsertElem(cItem);	// 将-bItem追加到lc的表尾
			bStatus = lb.GetElem(bPos++, bItem);// 取出lb的第下一项
		}
		else {	// la中的项aItem和lb中的项bItem指数相等
			PolyItem sumItem(aItem.coef - bItem.coef, aItem.expn);
			if (sumItem.coef != 0)
				lc.InsertElem(sumItem);	// 将两项的差追加到lc的表尾
			aStatus = la.GetElem(aPos++, aItem);// 取出la的第下一项
			bStatus = lb.GetElem(bPos++, bItem);// 取出lb的第下一项
		}
	}
	...
	while (bStatus == ENTRY_FOUND) {	// 将lb的剩余项追加到lc的后面
		cItem = bItem;
		cItem.coef = -cItem.coef;
		lc.InsertElem(cItem);	// 将-bItem追加到lc的后面
		bStatus = lb.GetElem(bPos++, bItem);// 取出lb的第下一项
	}
